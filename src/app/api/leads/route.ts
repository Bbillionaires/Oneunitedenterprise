...
async function resolvePipeline(sector: string): Promise<{ pipeline_id: string; stage_id: string }> {
  ...
  } catch (err) { console.error('[leads] pipeline lookup failed:', err) }
...
async function notifyNewLead(data: { name: string; email: string; company?: string; phone?: string; sector?: string; service?: string; message?: string }) {
  ...
  const tableRows = rows.map(([k, v]) => `<tr><td style="padding:6px 0;color:rgba(245,240,232,0.5);font-size:13px;width:90px">${k}</td><td style="padding:6px 0;font-size:13px">${v}</td></tr>`).join('')
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px;background:#12121F;color:#F5F0E8;border-radius:12px"><h2 style="color:#C9A84C;margin:0 0 8px">New Lead</h2><p style="color:rgba(245,240,232,0.5);font-size:13px;margin:0 0 24px">Someone submitted the contact form on oneunitedenterprise.com</p><table style="width:100%;border-collapse:collapse">${tableRows}</table><a href="${base}/admin/leads" style="display:inline-block;margin-top:24px;padding:10px 20px;background:#C9A84C;color:#07070F;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600">View in Admin</a></div>`,
    }),
  }).catch(err => console.error('[leads] email failed:', err))
}

export async function POST(req: NextRequest) {
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown'
  if (!checkRateLimit(ip)) return NextResponse.json({ error: 'Too many submissions. Please wait before trying again.' }, { status: 429 })
  ...
  const { pipeline_id, stage_id } = sector ? await resolvePipeline(sector) : { pipeline_id: PIPELINE_ID, stage_id: STAGE_ID }

  try {
    const res  = await fetch(`${AZOTH_URL}/api/contacts`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${AZOTH_SECRET}`, 'x-workspace-id': WORKSPACE_ID },
      body: JSON.stringify({ name, email, phone: phone||undefined, company: company||undefined, source: 'one-united-enterprise', pipeline_id, stage_id, tags, notes: message||undefined }),
    })
    const data = await res.json()
    if (!res.ok) { console.error('[leads] Azoth error:', data); return NextResponse.json({ error: 'CRM unavailable' }, { status: 502 }) }

    if (affiliate_code) {
      fetch(`${AZOTH_URL}/api/affiliate/lead`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${AZOTH_SECRET}` },
        body: JSON.stringify({ affiliate_code, workspace_id: WORKSPACE_ID, contact_name: name, contact_email: email, source_project: 'one-united-enterprise', metadata: { sector, service, plan } }),
      }).catch(err => console.error('[leads] affiliate failed:', err))
    }

    notifyNewLead({ name, email, company, phone, sector, service, message }).catch(console.error)
...
export async function GET(req: NextRequest) {
  ...
  const params = new URLSearchParams({
    pipeline: searchParams.get('pipeline') || PIPELINE_ID,
    stage:    searchParams.get('stage')    || STAGE_ID,
    limit:    searchParams.get('limit')    || '50',
    offset:   searchParams.get('offset')   || '0',
  })
  try {
    const res = await fetch(`${AZOTH_URL}/api/contacts?${params}`, {
      headers: { 'Authorization': `Bearer ${AZOTH_SECRET}`, 'x-workspace-id': WORKSPACE_ID },
    })
    const data = await res.json()
