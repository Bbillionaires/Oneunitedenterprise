...
export default function ContentPage() {
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-4xl font-light text-white mb-1">Content</h1>
      ...
      <GlassCard className="p-6 mb-6 border-gold/20" hover={false}>
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="font-body text-sm font-semibold text-white mb-1">Central Config File</h2>
        ...
        {SECTORS.map(sector => (
          <GlassCard key={sector.id} className="p-6" hover={false}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-body text-sm font-semibold text-white">{sector.name}</h3>
            </div>
            <div className="flex flex-col gap-2">
              <Link
                href={`/sector/${sector.id}`}
                target="_blank"
                className="flex items-center gap-2 font-body text-xs text-white/40 hover:text-white/70 transition-colors"
              >
              ...
              {sector.services.map(svc => (
                <Link
                  ...
                  target="_blank"
                  className="flex items-center gap-2 font-body text-xs text-white/30 hover:text-white/60 transition-colors ml-3"
                >
