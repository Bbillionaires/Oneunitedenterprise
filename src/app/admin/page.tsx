...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-4xl font-light text-gray-900 mb-1">Dashboard</h1>
        <p className="font-body text-sm text-gray-900/40">One United Enterprise LLC — Admin Overview</p>
      </div>
          ...
          return (
            <GlassCard key={card.label} className="p-6 flex flex-col gap-3" hover={false}>
              <div className="flex items-center justify-between">
                <span className="font-body text-xs text-gray-900/40 uppercase tracking-wide">{card.label}</span>
                <Icon size={16} style={{ color: card.color }} />
              </div>
              <div className="font-display text-3xl font-light text-gray-900">{card.value}</div>
            </GlassCard>
          )
        })}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GlassCard className="p-6" hover={false}>
          <h2 className="font-display text-xl text-gray-900 mb-4">Sector Overview</h2>
          <div className="flex flex-col gap-3">
            {SECTORS.map(s => (
              <div key={s.id} className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full" style={{ background: s.color }} />
                <span className="font-body text-sm text-gray-900/70 flex-1">{s.name}</span>
                <span className="font-body text-sm text-gray-900/30">{s.services.length} services</span>
              </div>
            ))}
          </div>
        </GlassCard>
          ...
          <div className="flex flex-col gap-3">
            {[{ label: 'View All Leads', href: '/admin/leads' }, { label: 'Edit Site Content', href: '/admin/content' }, { label: 'View Homepage', href: '/' }].map(a => (
              <Link key={a.href} href={a.href}
                className="font-body text-sm text-gray-900/60 hover:text-gray-900 transition-colors px-4 py-3 rounded-xl bg-white/3 hover:bg-white/7 border border-black/5">
                {a.label}
              </Link>
            ))}
          </div>
